...
var csvCrawler = require('../bin/backoffice/_csvCrawler.')
...
var dictionaryMatcher = {
    ...
    getSimilars: function (filePath, index, fileField, indexLabelField, indexIdField,callback) {
        ...
        async.series([
            ...
            ,
            function (callbackSeries) {
                async.eachSeries(data[0], function (line, callbackEach) {
                    if ((count++) % 100 == 0)
                        console.log(count)
                    ...
                    /*    query={
                        ...
                        elasticRestProxy.executePostQuery(index + "/_search", query, function (err, result) {
                            ...
                            if (result.error)
                                return callbackEach(result.error)
...
return callback(null,matchesMap)
    ...
    , merge: function (files,index, dir) {
        ...
        for (var file in files) {
            ...
            bigMatchesMap[fileObj.id]={label:fileObj.label,id:fileObj.id,matches:matches}
...
if (true) {
    ...
    dictionaryMatcher.getSimilars("D:\\NLP\\ontologies\\TEPDK\\tblCodification.csv", "tepdk_tags", "assetTagCode", "functionalLocationCode", "tagNamex",function(err, result){
...
if (false) {
    var files = {
        ...
        ,
        "Quantum_physicalClasses.txt": {id: "http://w3id.org/readi/rdl/D101001053", label: "Artefact"}
        ,
        "Quantum_discipline.txt": {id: "http://w3id.org/readi/z018-rdl/Discipline", label: "Discipline"}
        ,
        "Quantum_attribute.txt": {id: "http://standards.iso.org/iso/15926/part14/Quality", label: "Quality"}
        ,
        ...
        ,
        "Quantum_documentType.txt": {id: "http://w3id.org/readi/rdl/D101001188", label: "Document"}
